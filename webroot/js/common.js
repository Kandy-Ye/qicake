		...
		.extend(
				...
				(function() {
					...
					function camelize() {
						...
						if (len == 1)
							return parts[0];
						var camelized = this.charAt(0) == '-' ? parts[0]
...
(function(global) {
	var SETATTRIBUTE_IGNORES_NAME = (function() {
		...
		var isBuggy = elForm.elements ? (typeof elForm.elements.test == "undefined")
				: null;
...
Element.Methods = {
	...
	siblings : function(element) {
		element = $(element);
		return Element.previousSiblings(element).reverse().concat(
	...
	toggleClassName : function(element, className) {
		if (!(element = $(element)))
			return;
		return Element[Element.hasClassName(element, className) ? 'removeClassName'
			...
			return function(element) {
				if (element
					...
					if (t && (/^(?:object|applet|embed)$/i.test(t))) {
						...
						extendElementWith(element, Element.Methods.ByTag[t
								.toUpperCase()]);
					}
				}
				return element;
				...
				while (e && le != e && (/\S/).test(e)) {
					...
					for ( var i = 0; i < len; i++) {
						...
						if (m = e.match(ps[i].re)) {
							this.matcher
									.push(Object.isFunction(x[name]) ? x[name]
											(m) : new Template(x[name])
											.evaluate(m));
							e = e.replace(m[0], '');
							break;
						}
...
if (Prototype.BrowserFeatures.SelectorsAPI
	...
	Selector.CASE_INSENSITIVE_CLASS_NAMES = (function() {
		var div = document.createElement('div'), span = document
				.createElement('span');
		...
		.extend(
				Selector,
				{
							...
							'not' : function(m) {
								...
								while (e && le != e && (/\S/).test(e)) {
									...
									for ( var i = 0; i < len; i++) {
										name = p[i].name
										if (m = e.match(p[i].re)) {
											v = Object.isFunction(x[name]) ? x[name]
													(m)
													: new Template(x[name])
															.evaluate(m);
											exclusion
					...
					findElement : function(elements, expression, index) {
						...
						return Selector.matchElements(elements,
								expression || '*')[index || 0];
					},
...
var Client = {
	...
	load : function() {
		...
		if (!Client.version)
			Client.version = Client.detect.charAt(Client.current_place
					+ Client.current_string.length);
		if (!Client.OS) {
			$H( {
				linux : "Linux",
				x11 : "Unix",
...
var Effect = {
	...
	Transitions : {
		...
		flicker : function(pos) {
			var pos = ((-Math.cos(pos * Math.PI) / 4) + .75) + Math.random()
					/ 4;
			return pos > 1 ? 1 : pos;
		},
		wobble : function(pos) {
	...
	multiple : function(element, effect) {
		var elements;
		if (((typeof element == 'object') || Object.isFunction(element))
				&& (element.length))
			elements = element;
		else
	...
	toggle : function(element, effect, options) {
		...
		return Effect[Effect.PAIRS[effect][element.visible() ? 1 : 0]](element,
				Object.extend( {
					queue : {
						position : 'end',
		...
		.create(
				Enumerable,
				{
					...
					add : function(effect) {
						...
						var position = Object.isString(effect.options.queue) ? effect.options.queue
		...
		.create( {
			...
			loop : function(timePos) {
				if (timePos >= this.startOn) {
					...
					var pos = (timePos - this.startOn) / this.totalTime, frame = (pos * this.totalFrames)
							.round();
		...
		.create(
				Effect.Base,
				{
					initialize : function(object, from, to) {
						...
						var args = $A(arguments), method = args.last(), options = args.length == 5 ? args[3]
								: null;
						this.method = Object.isFunction(method) ? method
								.bind(object) : Object
								.isFunction(object[method]) ? object[method]
								.bind(object) : function(value) {
							object[method] = value
						};
...
Effect.Move = Class.create(Effect.Base, {
	...
	update : function(position) {
		this.element
				.setStyle( {
					left : (this.options.x * position + this.originalLeft)
							.round() + 'px',
				...
				Effect.Base,
				{
					...
					setup : function() {
						...
						this._base = $R(0, 2).map(
								function(i) {
									return parseInt(this.options.startcolor
											.slice(i * 2 + 1, i * 2 + 3), 16)
								}.bind(this));
						this._delta = $R(0, 2).map(
								function(i) {
									return parseInt(this.options.endcolor
											.slice(i * 2 + 1, i * 2 + 3), 16)
											- this._base[i]
								}.bind(this));
					},
...
Effect.ScrollTo = function(element) {
	var options = arguments[1] || {}, scrollOffsets = document.viewport
			.getScrollOffsets(), elementOffsets = $(element).cumulativeOffset();
	if (options.offset)
		elementOffsets[1] += options.offset;
	return new Effect.Tween(null, scrollOffsets.top, elementOffsets[1],
			options, function(p) {
...
Effect.DropOut = function(element) {
	element = $(element);
	var oldStyle = {
		top : element.getStyle('right'),
		left : element.getStyle('bottom'),
		opacity : element.getInlineOpacity()
	};
	return new Effect.Parallel( [ new Effect.Move(element, {
		...
		duration : 0.5,
		beforeSetup : function(effect) {
			effect.effects[0].element.show().makePositioned();
		},
		afterFinishInternal : function(effect) {
...
Effect.Shake = function(element) {
	...
	var split = parseFloat(options.duration) / 15.0;
	var oldStyle = {
...
Effect.Pulsate = function(element) {
	element = $(element);
	var options = arguments[1] || {}, oldOpacity = element.getInlineOpacity(), transition = options.transition
			|| Effect.Transitions.linear, reverser = function(pos) {
		return 1 - transition((-Math.cos((pos * (options.pulses || 5) * 2)
				* Math.PI) / 2) + .5);
	};
								...
								.map(
										function(pair) {
											...
											if (value.parseColor('#zzzzzz') != '#zzzzzz') {
												...
												unit = (components.length == 3) ? components[2]
														: null;
											}
							...
							style[(transform = this.transforms[i]).style] = transform.unit == 'color' ? '#'
									...
									+ (Math
											.round(transform.originalValue[2]
													...
													* position)).toColorPart()
									: (transform.originalValue + (transform.targetValue - transform.originalValue)
											* position).toFixed(3)
											+ (transform.unit === null ? ''
													: transform.unit);
...
Effect.Transform = Class.create( {
	...
	play : function() {
		return new Effect.Parallel(this.tracks
				.map(
						function(track) {
							var ids = track.get('ids'), effect = track
									.get('effect'), options = track
									.get('options');
...
Effect.Methods = {
	...
	visualEffect : function(element, effect, options) {
		element = $(element);
		var s = effect.dasherize().camelize(), klass = s.charAt(0)
				.toUpperCase()
				+ s.substring(1);
...
var Droppables = {
	...
	isAffected : function(point, element, drop) {
		return ((drop.element != element)
				...
				&& ((!drop.accept) || (Element.classNames(element)
						.detect(function(v) {
							return drop.accept.include(v)
						}))) && Position.within(drop.element, point[0],
				point[1]));
	},
	...
	show : function(point, element) {
		...
		if (drop) {
			...
			if (drop.onHover)
				drop.onHover(element, drop.element, Position.overlap(
						drop.overlap, drop.element));
...
var Draggables = {
	...
	updateDrag : function(event) {
		...
		if (this._lastPointer
				&& (this._lastPointer.inspect() == pointer.inspect()))
			return;
	...
	_cacheObserverCallbacks : function() {
		[ 'onStart', 'onEnd', 'onDrag' ].each(function(eventName) {
			Draggables[eventName + 'Count'] = Draggables.observers
					.select(function(o) {
						return o[eventName];
					}).length;
		});
			...
			finishDrag : function(event, success) {
				...
				if (revert && this.options.reverteffect) {
					if (dropped == 0 || revert != 'failure')
						this.options.reverteffect(this.element, d[1]
								- this.delta[1], d[0] - this.delta[0]);
				} else {
					this.delta = d;
				}
				if (this.options.zindex)
...
var Sortable = {
	...
	create : function(element) {
		...
		(options.elements || this.findElements(element, options) || [])
				.each(function(e, i) {
					var handle = options.handles ? $(options.handles[i])
							: (options.handle ? $(e).select(
									'.' + options.handle)[0] : e);
					options.draggables.push(new Draggable(e, Object.extend(
		...
		if (options.tree) {
			(Sortable.findTreeElements(element, options) || [])
					.each(function(e) {
	...
	findElements : function(element, options) {
		return Element.findChildren(element, options.only, options.tree ? true
				: false, options.tag);
	},
	...
	sequence : function(element) {
		...
		return $(this.findElements(element, options) || []).map(
				function(item) {
					return item.id.match(options.format) ? item.id
							.match(options.format)[1] : '';
				});
	},
	...
	serialize : function(element) {
		...
		var options = Object.extend(Sortable.options(element), arguments[1]
				|| {});
		var name = encodeURIComponent((arguments[1] && arguments[1].name) ? arguments[1].name
				: element.id);
		if (options.tree) {
	...
	$A(element.childNodes).each(
			function(e) {
				if (e.tagName && e.tagName.toUpperCase() == tagName
					...
					elements.push(e);
				if (recursive) {
					var grandchildren = Element.findChildren(e, only,
							recursive, tagName);
					if (grandchildren)
...
var Builder = {
				...
				if (attrs.length) {
					...
					if (element.tagName.toUpperCase() != elementName)
						element = parentElement
								.getElementsByTagName(elementName)[0];
				}
			}
		if (arguments[2])
	...
	dump : function(scope) {
		...
		tags.each(function(tag) {
			scope[tag] = function() {
				return Builder.node.apply(Builder, [ tag ]
						.concat($A(arguments)));
			};
		});
	}
...
function init() {
	...
	activeButtons();
	ifSystemAlert();
	setTimeout('resetSession()', 900000);
}
function toggleSystemAlert() {
	Effect.toggle('infoX', 'slide', {
		duration : .3,
		afterFinish : function() {
			showFlash();
		}
...
function hideFlash() {
	...
	Element.addClassName(bod, 'hideFlash');
}
function showFlash() {
	bod = document.getElementsByTagName('body')[0];
	Element.removeClassName(bod, 'hideFlash');
}
function activeButtons() {
	var buttons = $$('button.button', '.buttons button');
...
function initToolTips() {
	...
	for ( var i = 0; i < tooltips.length; i++) {
		tooltips[i].stopObserving();
		Event.observe(tooltips[i], 'click', function(e) {
			toolTipToggle(this, e);
...
function toolTipToggle(ctrl, e) {
	mX = Event.pointerX(e);
	mY = Event.pointerY(e);
	(ctrl.nextSibling && ctrl.nextSibling.id == 'tooltip') ? Element
			.removeTitle(ctrl.nextSibling) : Element.showTitle(ctrl);
}
Element.showTitle = function(element) {
	if ($("tooltip"))
		Element.removeTitle($("tooltip"));
	else {
		var title = Builder.node('span', {
			style : 'visibility:hidden',
			id : 'tooltip',
			onclick : 'Element.removeTitle (this)'
		},
		...
		title.style.top = mY + 'px';
		title.style.left = mX + 'px';
		bod = document.getElementsByTagName('body')[0];
		bod.appendChild(title);
		var titleW = title.offsetWidth + 'px';
		var titleH = title.offsetHeight + 'px';
		title.style.visibility = 'visible';
		title.style.display = 'none';
		if (browser == "Internet Explorer 6") {
			var ifr = Builder.node('iframe', {
				id : 'tipFrame',
				width : titleW,
				height : titleH,
				border : '0'
...
Element.removeTitle = function(element) {
	Effect.Fade(element, {
		afterFinish : function() {
...
function getPlatform() {
	if (checkIt('konqueror')) {
		...
		browser = "Firefox";
	else if (checkIt('msie 6'))
		browser = "Internet Explorer 6";
	else if (checkIt('msie 7'))
	...
	setTimeout(function() {
		if ($('y'))
			Effect.DropOut('y', {
				duration : 1,
				afterFinish : function() {
					$('statusText').innerHTML = 'Loading Data ...';
				}
			});
	}, 550);
}
...
function createCookieWithExpirationLimit(name, value, days) {
	var date = new Date();
...
function resetSession() {
	var myAjax = new Ajax.Request('/admin/reset/', {
		method : 'post',
		parameters : '',
		onComplete : function() {
			setTimeout('resetSession()', 900000);
...
function unlockSession() {
	var myAjax = new Ajax.Request('/global/unlock/', {
		asynchronous : false,
		method : 'post',
...
var Lightbox = {
	...
	lightBoxType : '',
	showUrl : function(url, completed, type, caption) {
		this.lightBoxType = type;
		this.checkLink(url);
		if ($('statusText')) {
			$('statusText').innerHTML = 'Loading Lightbox ...';
		}
		if (this.lightBoxType == 'image') {
			content = '<img id="lbImage" class="lbOff" src="' + url + '" alt="" />';
			if (caption)
				content += '<h4 id="lbCaption">' + caption + '</h4>';
			content += '<div><a class="lbOff button dark" href="#"><img src="/images/icons/cross.png" alt=""/> Close Image</a></div>';
			Lightbox.showContent(content, completed);
		} else if (this.lightBoxType == 'movie') {
			var replace = new Array('http://vzaar.com/videos/',
					'http://view.vzaar.com/', '.video', '.swf', '.flashplayer');
			for ( var i = 0; i < replace.length; i++) {
				url = url.replace(replace[i], '');
			}
			content = '<div id="lbMovie">';
			content += '<object id="video" width="768" height="432" type="application/x-shockwave-flash" data="http://view.vzaar.com/' + url + '.flashplayer">';
			content += '<param name="movie" value="http://view.vzaar.com/' + url + '.flashplayer">';
			...
			content += '<param name="flashvars" value="border=none&autoplay=true">';
			content += '<embed src="http://view.vzaar.com/' + url + '.flashplayer" type="application/x-shockwave-flash" wmode="transparent" width="768" height="432" allowScriptAccess="always" allowFullScreen="true" flashvars="border=none&autoplay=true"></embed>';
			content += '<video width="1280" height="720" src="http://view.vzaar.com/'
					+ url
					+ '.mobile" poster="http://view.vzaar.com/'
					+ url
					+ '.image" controls onclick="this.play();"></video></object>';
			content += '</div>';
			if (caption)
				content += '<h4 id="lbCaption">' + caption + '</h4>';
			content += '<div><a class="lbOff button dark" href="#"><img src="/images/icons/cross.png" alt=""/> Close Video</a></div>';
			Lightbox.showContent(content, completed);
		} else {
			var myAjax = new Ajax.Request(url, {
				...
				onComplete : function(response) {
					Lightbox.showContent(response.responseText, completed);
				}
			});
		}
	},
	pageUrl : function(url, completed) {
		if (!completed) {
			completed = function() {
			}
		}
	...
	showContent : function(content, completed) {
		...
		this.resizeImage();
		if (this.lightBoxType == 'image') {
			var t = setTimeout("Lightbox.center();", 50);
		} else {
			this.center();
		}
		if (!completed)
	...
	addMarkup : function() {
		...
		bod = document.getElementsByTagName('body')[0];
		$(bod).addClassName('lb');
		box = Builder.node('div', {
			id : 'lightbox',
			className : 'loadin'
		}, [ Builder.node('div', {
			id : 'lbLoadMessage'
		}, [ Builder.node('p', 'Loading...') ]) ]);
		bod.appendChild(box);
	},
	addOverlay : function() {
		if (Prototype.Browser.IE)
			this.prepareIE();
		bod = document.getElementsByTagName('body')[0];
		(this.lightBoxType == 'modal' || this.lightBoxType == 'error') ? overlay = Builder
				.node('div', {
					id : 'overlay'
				})
				: overlay = Builder.node('div', {
					id : 'overlay',
					className : 'lbOff'
				});
	...
	show : function(content) {
		newcontent = '<div id="lbContent">' + content + '</div>';
		new Insertion.Before($('lbLoadMessage'), newcontent);
		$('lightbox').style.visibility = 'hidden';
		$('lightbox').className = "done " + this.lightBoxType;
	},
	close : function() {
		var boxed = $$('.boxed');
		if (boxed.each) {
			boxed.each(function(num, index) {
				num.hide();
			});
		}
		if ($('lightbox')) {
			Element.remove(box);
			bod.removeClassName('lb');
		}
		if ($('overlay')) {
			Element.remove(overlay);
			if (Prototype.Browser.IE) {
				Lightbox.unprepareIE();
			}
		}
		this.lightBoxShowing = false;
	},
	checkLink : function(linkHref) {
		...
		if (extension in {
			...
			this.lightBoxType = 'movie';
		}
	},
	center : function() {
		var lbHeight = $('lightbox').offsetHeight;
		var lbWidth = $('lightbox').offsetWidth;
		$('lightbox').style.marginTop = '-' + (lbHeight / 2) + 'px';
		$('lightbox').style.marginLeft = '-' + (lbWidth / 2) + 'px';
		$('lightbox').style.visibility = 'visible';
	},
	resizeImage : function() {
		if ($('lbImage')
				&& document.viewport.getHeight() <= $('lbImage').offsetHeight) {
			$('lbImage').style.height = document.viewport.getHeight() * .75 + 'px';
		}
	},
	prepareIE : function() {
		el = document.getElementsByTagName('html')[0];
		prevScroll = this.getScroll();
		window.scrollTo(0, 0);
		Element.addClassName(el, 'prepIE');
	},
	unprepareIE : function() {
		el = document.getElementsByTagName('html')[0];
		Element.removeClassName(el, 'prepIE');
...
Event.observe(window, 'load', attachLightbox, false);
function lightbox(title, message, type) {
	markup = '<form class="prompt"><h2>' + title + '</h2>';
	markup += '<p>' + message + '</p>';
	markup += '<a href="#" class="lbOff button negative"><img src="/images/icons/cross.png" alt=""/> Close</a></form>';
	Lightbox.showContent(markup, '', type);
}
function lightboxElement(id, type) {
	Lightbox.lightBoxType = type;
	Lightbox.addOverlay();
	if ($(id))
		$(id).show();
	attachCloseLightbox();
...
function attachLightbox() {
	var on = $$('.lbOn');
	if (on.each) {
		on.each(function(num, index) {
			num.onclick = function() {
				Lightbox.showUrl(num.href, '', '', num.title);
				return false
			};
		});
	}
}
function attachCloseLightbox() {
	var off = $$('.lbOff');
	if (off.each) {
		off.each(function(num, index) {
			Event.observe(num, 'click', Lightbox.close, false);
			num.onclick = function() {
				return false
			};
		});
...
validation.prototype = {
	initialize : function(ctrl, oncomplete, dontBind) {
		this.isSubmitting = false;
		this.form = ctrl;
		this.getHiddenFields();
		this.oncomplete = oncomplete;
		this.event = this.prepare.bind(this);
		if (!dontBind) {
			Event.observe(this.form, 'submit', this.event, false);
	...
	prepare : function() {
		if (!this.isSubmitting) {
			...
			this.submitting();
			var serverCall = Form.serialize(this.form);
			serverCall += "&ajax=true";
			this.validate(serverCall);
		}
	},
	getHiddenFields : function() {
		var hidden = Form.getInputs(this.form, "hidden");
		for (i = 0; i < hidden.length; i++) {
			this[hidden[i].name] = hidden[i].value;
		}
	},
	validate : function(serverCall) {
		var myAjax = new Ajax.Request(this.iFlowLoc, {
			method : 'post',
			parameters : serverCall,
			onComplete : this.process.bindAsEventListener(this)
		});
	},
	process : function(response) {
		...
		if (!response.responseText
				|| (response.responseText && response.responseText.substring(0,
						4) == 'safe')) {
			if (this.oncomplete)
				this.oncomplete(response.responseText);
			else if (this.iLbRedirect) {
				(Element.hasClassName(this.iLbRedirect, 'show')) ? Lightbox
						.showUrl(this.iLbRedirect, function() {
							refreshLightbox();
						}) : Lightbox.pageUrl(this.iLbRedirect, function() {
					refreshLightbox();
				});
			} else
				window.location = this.iRedirect;
		} else
	...
	removeErrors : function() {
		...
		if ($('errorMsgLbl'))
			Element.remove('errorMsgLbl');
		if ($('liError'))
			Element.remove('liError');
		allErrors = document.getElementsByClassName("error");
		for (i = 0; i < allErrors.length; i++) {
			if (!Element.hasClassName(allErrors[i], 'file')
					&& !Element.hasClassName(allErrors[i].parentNode, 'file')) {
				Element.removeClassName(allErrors[i], "error");
				if (allErrors[i].tagName == "P" || allErrors[i].tagName == "H3") {
					allErrors[i].innerHTML = "";
				}
			}
		}
	},
	parseResponse : function(serverResponse) {
		var nameMsgPairs = serverResponse.split("|");
		for (i = 0; i < nameMsgPairs.length; i++) {
			if (nameMsgPairs[i] != "") {
				var nameMsgPair = nameMsgPairs[i].split(",");
				name = nameMsgPair[0];
				msg = nameMsgPair[1];
				inputs = $(name).parentNode.parentNode
						.getElementsByTagName('input');
				textareas = $(name).parentNode.parentNode
						.getElementsByTagName('textarea');
				selects = $(name).parentNode.parentNode
						.getElementsByTagName('select');
				this.drawErrorMessage();
				this.addErrors(name);
				this.textAreaErrors(name, msg, textareas);
				this.inputErrors(name, msg, inputs);
				this.selectErrors(name, msg, selects);
				if (inputs.length == 5) {
					this.checkAddress(name, inputs);
				}
				window.scrollTo(0, 0);
			}
		}
	...
	drawErrorMessage : function() {
		if ($('errorMsgLbl')) {
			Element.addClassName($('errorMsgLbl').parentNode, "error");
		}
		if (!$('errorMsgLbl') && $('formFields')) {
			errorMsg = '<li id="liError" class="error"><h3 class="error" id="errorMsgLbl">There was a problem with your submission.</h3><p id="errorMsg">Errors have been highlighted below.</p></li>'
			new Insertion.Before($('formFields').childNodes[0], errorMsg);
		}
	},
	addErrors : function(name) {
		Element.addClassName($(name).parentNode.parentNode, "error");
		$(name).parentNode.parentNode.lastChild.className = "error";
		Element.addClassName($(name), "error");
	},
	textAreaErrors : function(name, msg, textareas) {
		if (textareas.length > 0) {
			$(name).parentNode.parentNode.lastChild.innerHTML = msg;
		}
	},
	inputErrors : function(name, msg, inputs) {
		for (x = 0; x < inputs.length; x++) {
			if (inputs.length < 4) {
				Element.addClassName($(inputs[x].id), "error");
				$(name).parentNode.parentNode.lastChild.innerHTML = msg;
			}
		}
	},
	selectErrors : function(name, msg, selects) {
		for (x = 0; x < selects.length; x++) {
			Element.addClassName($(selects[x].id), "error");
			$(name).parentNode.parentNode.lastChild.innerHTML = msg;
		}
	},
	checkAddress : function(name, inputs) {
		$(name).parentNode.parentNode.lastChild.innerHTML = "Please Enter a Valid Address";
	}
}
function initializeValidation() {
	var forms = document.getElementsByTagName('form');
	for (i = 0; i < forms.length; i++) {
		valid = new validation(forms[i]);
	}
}
